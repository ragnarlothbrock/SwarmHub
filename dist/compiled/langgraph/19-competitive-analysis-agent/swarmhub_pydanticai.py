...
identify_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted automatically via SwarmHub static AST source-slicing. Node context: identify. Authorized MCP Interfaces: []",
)

analyze_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted automatically via SwarmHub static AST source-slicing. Node context: analyze. Authorized MCP Interfaces: []",
)

report_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted automatically via SwarmHub static AST source-slicing. Node context: report. Authorized MCP Interfaces: []",
)
...
if __name__ == "__main__":
    ...
    NODES_PIPELINE_MAP = {
        "identify": importlib.import_module("blobs.identify").run,
        "analyze": importlib.import_module("blobs.analyze").run,
        "report": importlib.import_module("blobs.report").run,
    }
