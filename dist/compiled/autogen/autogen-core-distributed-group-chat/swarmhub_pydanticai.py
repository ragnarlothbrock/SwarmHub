...
BaseGroupChatAgent_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: BaseGroupChatAgent. Authorized MCP Interfaces: []",
)
...
UIAgent_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: UIAgent. Authorized MCP Interfaces: []",
)
...
if __name__ == "__main__":
    ...
    NODES_REGISTRY = {
        "BaseGroupChatAgent": BaseGroupChatAgent_agent,
        "GroupChatManager": GroupChatManager_agent,
        "UIAgent": UIAgent_agent,
    }
    NODES_PIPELINE_MAP = {
        "BaseGroupChatAgent": importlib.import_module("blobs.BaseGroupChatAgent").run,
        "GroupChatManager": importlib.import_module("blobs.GroupChatManager").run,
        "UIAgent": importlib.import_module("blobs.UIAgent").run,
    }
