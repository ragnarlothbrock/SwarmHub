...
AIAgent_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: AIAgent. Authorized MCP Interfaces: []",
)

UserAgent_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: UserAgent. Authorized MCP Interfaces: []",
)
...
if __name__ == "__main__":
    ...
    NODES_REGISTRY = {
        ...
        "UserAgent": UserAgent_agent,
    }
    NODES_PIPELINE_MAP = {
        "AIAgent": importlib.import_module("blobs.AIAgent").run,
        "UserAgent": importlib.import_module("blobs.UserAgent").run,
    }
