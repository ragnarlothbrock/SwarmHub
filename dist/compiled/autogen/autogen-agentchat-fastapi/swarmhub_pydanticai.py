...
yoda_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: yoda. Authorized MCP Interfaces: []",
)

user_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: user. Authorized MCP Interfaces: []",
)
...
if __name__ == "__main__":
    ...
    NODES_PIPELINE_MAP = {
        ...
        "yoda": importlib.import_module("blobs.yoda").run,
        "user": importlib.import_module("blobs.user").run,
        "assistant": importlib.import_module("blobs.assistant").run,
    }
