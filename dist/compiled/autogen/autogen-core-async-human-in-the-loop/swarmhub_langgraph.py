# ==========================================================================
# GENERATED AUTOMATICALLY BY SWARMHUB FOR AGENT: autogen-core-async-human-in-the-loop
...
def SlowUserProxyAgent(state: AgentState) -> AgentState:
    print(f"\n--- 🟢 Entering Runtime Node: SlowUserProxyAgent ---")
    ...
    try:
        ...
        print(f"    ⚠️ [Contract Breakage] Incoming schema anomaly caught at entry of SlowUserProxyAgent: {contract_err}")
        contract_status = "FAILED_ENTRY"
    try:
        module = importlib.import_module("blobs.SlowUserProxyAgent")
        ...
        if contract_status == "VERIFIED":
            try:
                ...
                print(f"    ❌ [Contract Breakage] Outgoing contract violation caught at exit of SlowUserProxyAgent: {contract_err_exit}")
                contract_status = "FAILED_EXIT"
    except Exception as e:
        print(f"    ❌ Execution/Contract Fault inside SlowUserProxyAgent: {e}")
...
def SchedulingAssistantAgent(state: AgentState) -> AgentState:
    print(f"\n--- 🟢 Entering Runtime Node: SchedulingAssistantAgent ---")
    print(f"    🔒 [Permissions] Authorized MCP Interfaces: []")
    span_id = f"span-{uuid.uuid4().hex[:8]}"
    contract_status = "VERIFIED"
    incoming_action = state.get("next_action", "PROCEED") or "PROCEED"
    ...
    try:
        ...
        print(f"    ⚠️ [Contract Breakage] Incoming schema anomaly caught at entry of SchedulingAssistantAgent: {contract_err}")
        contract_status = "FAILED_ENTRY"
    try:
        module = importlib.import_module("blobs.SchedulingAssistantAgent")
        state = module.run(state)
        if contract_status == "VERIFIED":
            try:
                SharedContextContract(**state.get("context", {}))
            except Exception as contract_err_exit:
                print(f"    ❌ [Contract Breakage] Outgoing contract violation caught at exit of SchedulingAssistantAgent: {contract_err_exit}")
                contract_status = "FAILED_EXIT"
    except Exception as e:
        print(f"    ❌ Execution/Contract Fault inside SchedulingAssistantAgent: {e}")
        contract_status = "CRASHED"
    latency = round(time.perf_counter() - start_time, 4)
    outgoing_action = state.get("next_action", "PROCEED") or "PROCEED"
    if outgoing_action.upper().startswith("GOTO_"):
    ...
    telemetry_span = {
        "span_id": span_id,
        "thread_id": "swarmhub-default-thread",
        "node_id": "SchedulingAssistantAgent",
        "latency_seconds": latency,
        "incoming_action": incoming_action.upper(),
        "outgoing_action": outgoing_action.upper(),
        "contract_status": contract_status
...
workflow.add_node("SlowUserProxyAgent", SlowUserProxyAgent)
workflow.add_node("SchedulingAssistantAgent", SchedulingAssistantAgent)

workflow.add_edge(START, "SlowUserProxyAgent")
...
workflow.add_conditional_edges(
    ...
    route_normalizer,
    {
        "GOTO_SCHEDULINGASSISTANTAGENT": "SchedulingAssistantAgent",
        "GOTO_END": END,
        "END": END
    }
)
workflow.add_edge("SchedulingAssistantAgent", END)
