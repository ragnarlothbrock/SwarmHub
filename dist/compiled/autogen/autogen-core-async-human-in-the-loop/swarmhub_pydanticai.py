...
SlowUserProxyAgent_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: SlowUserProxyAgent. Authorized MCP Interfaces: []",
)

SchedulingAssistantAgent_agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt="Extracted from legacy conversational script via AST source-slicing. Node context: SchedulingAssistantAgent. Authorized MCP Interfaces: []",
)
...
if __name__ == "__main__":
    ...
    NODES_REGISTRY = {
        "SlowUserProxyAgent": SlowUserProxyAgent_agent,
        "SchedulingAssistantAgent": SchedulingAssistantAgent_agent,
    }
    NODES_PIPELINE_MAP = {
        "SlowUserProxyAgent": importlib.import_module("blobs.SlowUserProxyAgent").run,
        "SchedulingAssistantAgent": importlib.import_module("blobs.SchedulingAssistantAgent").run,
    }
