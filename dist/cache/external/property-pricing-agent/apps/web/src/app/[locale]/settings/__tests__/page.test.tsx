...
jest.mock('@/components/settings/task-model-settings', () => ({
  TaskModelSettings: () => (
    <div data-testid="task-model-settings">Task Model Settings Component</div>
  ),
}));
...
describe('SettingsPage', () => {
  ...
  it('renders page title and description', () => {
    ...
    expect(
      screen.getByText('Manage your account settings and notification preferences.')
    ).toBeInTheDocument();
  });
  ...
  it('shows retry on catalog load failure', async () => {
    ...
    mockGetModelsCatalog.mockRejectedValueOnce(
      new ApiError('Failed to load model catalog. Please try again.', 500)
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Failed to load model catalog. Please try again.')
      ).toBeInTheDocument();
    });
  ...
  it('renders local runtime available status and downloaded models list', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Local runtime detected\. Downloaded models: 2\./i)
      ).toBeInTheDocument();
    });
